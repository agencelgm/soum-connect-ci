...
-- Empêche double unlock du même lead par le même partenaire
CREATE UNIQUE INDEX IF NOT EXISTS lead_unlocks_partner_publication_unique
  ON public.lead_unlocks (partner_id, publication_id);

-- updated_at trigger sur lead_publications
DROP TRIGGER IF EXISTS set_lead_publications_updated_at ON public.lead_publications;
CREATE TRIGGER set_lead_publications_updated_at
  BEFORE UPDATE ON public.lead_publications
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

-- RPC : déverrouille un lead pour le partenaire courant
...
BEGIN
  -- Récupère le partenaire actif du caller
  ...
  -- Idempotence : si déjà débloqué, renvoie le prospect sans débiter
  IF EXISTS (
  ...
  -- Débit + insert unlock + incr compteur
  ...
  INSERT INTO public.credit_transactions (partner_id, amount, balance_after, tx_type, reference_id, note, created_by)
    VALUES (v_partner.id, -1, v_new_balance, 'lead_unlock', v_pub.id, 'Déblocage lead', auth.uid());
...
-- RPC staff : publie un prospect comme lead anonymisé
