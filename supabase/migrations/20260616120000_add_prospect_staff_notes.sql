...
COMMENT ON COLUMN public.prospects.external_notes IS 'Staff-written note visible to approved partners through marketplace lead summaries.';
COMMENT ON COLUMN public.prospects.internal_notes IS 'Staff-only note for internal qualification follow-up.';
COMMENT ON COLUMN public.prospects.edited_by IS 'Last staff user who edited the normalized prospect fields.';
COMMENT ON COLUMN public.prospects.edited_at IS 'Last staff edit timestamp for normalized prospect fields.';
...
BEGIN
  ...
  INSERT INTO public.lead_publications (
    ...
    v_prospect.budget,
    NULLIF(BTRIM(COALESCE(_summary, v_prospect.external_notes, v_prospect.message, '')), ''),
